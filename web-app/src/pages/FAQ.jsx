...
export default function FAQ() {
  ...
  return (
    <div className="min-h-screen bg-white text-[#1d1d1f]">
      ...
      {/* HERO */}
      <div className="pt-40 pb-20 px-6 md:px-12 relative overflow-hidden"
        style={{ backgroundImage: `url('${import.meta.env.BASE_URL}images/flat.jpg')`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className="max-w-[1400px] mx-auto relative z-10">
          ...
          <h1 className="text-5xl md:text-8xl font-display font-bold tracking-tighter leading-none text-white mb-8">FAQ &amp;<br /><span className="text-[#ea5d1a]">Manutenzione.</span></h1>
          <div className="w-20 h-[2px] bg-[#ea5d1a] mb-8" />
          <p className="text-white/70 text-lg font-light leading-relaxed max-w-2xl">Tutte le risposte alle domande più comuni, il piano di manutenzione programmata e le avvertenze tecniche per le tue porte Quadra.</p>
        </div>
      </div>
      ...
      <div className="bg-[#0c0c0d] py-12 px-6 md:px-12">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <p className="text-white/90 text-sm font-light tracking-wide">Documentazione tecnica ufficiale Quadra S.r.l. — aggiornata al catalogo 2026.</p>
      ...
      <div className="py-20 px-6 md:px-12 bg-white">
        <div className="max-w-[1400px] mx-auto">
          <div className="mb-12">
            ...
            <p className="text-[#444] text-base font-light mt-3 max-w-2xl">Le porte Quadra sono progettate per durare oltre 20 anni. Segui il piano di manutenzione per garantire prestazioni ottimali nel tempo.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-6 mb-14">
            {maintenanceSteps.map((s, i) => (
              <div key={i} className="bg-[#f5f5f7] rounded-2xl p-7 border border-[#e0e0e0]">
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-3 h-3 rounded-full" style={{ backgroundColor: s.color, boxShadow: `0 0 8px ${s.color}80` }} />
                  <span className="text-sm font-bold uppercase tracking-widest" style={{ color: s.color }}>{s.interval}</span>
                </div>
                <ul className="flex flex-col gap-3">
                  {s.tasks.map((t, j) => (
                    <li key={j} className="flex items-start gap-2 text-sm text-[#333] font-light leading-relaxed">
                      <CheckCircle size={14} className="shrink-0 mt-0.5" style={{ color: s.color }} />{t}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="flex items-start gap-4 bg-amber-50 border border-amber-200 rounded-2xl p-6 mb-6">
            ...
            <div>
              ...
              <p className="text-amber-800/70 text-sm font-light leading-relaxed">Sebbene il telaio Quadra in HDF idrofugo garantisca elevata resistenza all'umidità, evitare l'esposizione prolungata ad ambienti con umidità relativa superiore all'80%. In caso di allagamenti o infiltrazioni, contattare immediatamente il rivenditore per una verifica strutturale prima di continuare l'utilizzo.</p>
            </div>
          </div>
          <div className="flex items-start gap-4 bg-blue-50 border border-blue-200 rounded-2xl p-6">
            ...
            <div>
              ...
              <p className="text-blue-800/70 text-sm font-light leading-relaxed">Tutti i prodotti Quadra S.r.l. sono coperti da garanzia legale di conformità di 24 mesi dalla data di acquisto. La garanzia copre difetti di materiale e fabbricazione, escludendo i danni da uso improprio o mancata manutenzione. Per assistenza: <strong>assistenza@quadraporte.it</strong></p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-3 max-w-3xl mx-auto">
            {faqItems.map((item, i) => (
              <div key={i} className="bg-white rounded-2xl border border-[#e0e0e0] overflow-hidden shadow-sm">
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-start justify-between p-6 text-left cursor-pointer hover:bg-[#f5f5f7] transition-colors gap-4">
                  <div>
                  ...
                  <ChevronDown size={18} className={`text-[#ea5d1a] shrink-0 mt-1 transition-transform duration-300 ${openFaq === i ? 'rotate-180' : ''}`} />
                </button>
                {openFaq === i && (
        ...
        <div className="max-w-[1400px] mx-auto">
          <div className="bg-[#1d1d1f] rounded-[2rem] p-10 md:p-14 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
            <div>
              ...
              <p className="text-white/70 text-sm font-light leading-relaxed max-w-lg">Il nostro team tecnico è disponibile per rispondere a qualsiasi domanda sulla tua porta Quadra. Scrivici o chiamaci.</p>
            </div>
            <Link to="/#contatti" className="shrink-0 px-8 py-4 bg-[#ea5d1a] rounded-full text-white font-bold text-sm uppercase tracking-widest hover:bg-[#c94e15] transition-colors shadow-lg shadow-[#ea5d1a]/20">Contattaci</Link>
          </div>
        </div>
      </div>
